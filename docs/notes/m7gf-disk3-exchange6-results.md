...
[m7gg](m7gg-data-disk-screening.md)が10本のディスクをB:として選別した
際、`disk#3`（`c29e67d4`）と`disk#8`（`650cfac8`）は、B:ディレクトリ
表示の画面行数（17行）・文字数（756文字）・画面出力SHA-256
（`da678af6`）・`READ DATA`件数（7件）のすべてが一致した。他の組には
この一致は見られない（`disk#1`・`#2`・`#4`・`#5`・`#7`・`#9`・`#10`の
画面出力SHA-256は互いに全て異なる）。

**したがって本稿の結論のうち、「`disk#3`という新しい起動ディスクでも
交換#6の目的シリンダは偶数だった」「新しい刺激の上でも公式ROM一式との
適合が成立した」という部分は、独立な2本目の起動ディスクによる裏付けと
しては数えられない。** `disk#3`は`disk#8`と実質同一内容を測り直した
可能性が高い。上の「言えないこと」節が「`disk#3`と`disk#8`が同一系統の
ディスクなのか…本稿の測定からは分からない」と書いていた点は、`m7gg`に
よって（別の測定を通じて）判明したことになる。

ただし限界がある。`m7gg`が一致を確認したのは**B:ディレクトリ表示の
署名**（画面行数・文字数・SHA-256・READ DATA件数）であって、
`disk#3`と`disk#8`のディスクイメージそのもののバイト比較ではない。
**ディレクトリ表示が同じであることから、2枚の物理配置（トラック・
セクタ配置）まで同一であるとは、この測定からは言えない。** 「実質同一
内容と判断される」以上の強い主張はしない。

**本稿の測定手続き自体は無効にならない。** 事前登録（`m7ge`）が測定前に
コミットされていたこと、段階2の陽性対照が通ったこと、判定規則をまず
ベースラインへ当てたこと（決定論性の確認）は、いずれもそのまま成立
している。弱まったのは、それらの手続きで得られた結果が「独立な2本目の
起動ディスクによる追試」という位置づけを持てるかどうか、その一点だけ
である。
