...
「最初に違うセル」節で挙げた1桁ずれの原因は、自作ROMのキー処理が
SPACEキーでカーソルを前進させない実装だったこと（`print`のあとの
半角スペース1文字ぶん、打鍵エコーが詰まっていた）。`fc69064`
（M7: SPACEキーのエコー前進を実装、仕様書`l3-main.md`第3.1版第9節
`1439d78`に基づく）で修正済み。本追記の時点でP1〜P5を測り直しては
いない（測り直しは下記`l4-c2b`で行う）。

### P6（2腕）の診断とマニュアル確認

P6のエラー行署名不一致について、公式ROMのみで再測定・診断した
（`70de1bd`）。`docs/spec/l4-basic.md`6.1節のエラーメッセージ一覧51件
から表記ゆれ4通り（`plain`/`plain_upper`/`qmark`/`qmark_upper`）の
204候補を作り、公式ROM側の`row_sha256`と突き合わせたが、いずれとも
一致しなかった。**結論: 公式ROMのエラーの行は、マニュアルの一覧の
文言だけの行ではない。**

続けてマニュアル本文（直接モードのエラー表示の書式規則、および
Missing operand・Syntax errorの意味/原因欄）を確認した
（`1e1c119`・`02f30d6`）。資料6冒頭（PDF頁382、資ー6）をページ画像で
確認した結果、ダイレクトモードの書式は文言そのもの（前後に記号無し）、
プログラムモードは「文言 in 行番号」であり、OCRの欠落ではなく組版
どおりと確認できた。しかしこの書式規則からは、観測された非空白件数
（P6-syntax=15、P6-unknown=12。いずれも一覧文言の文字数に対して+1）を
説明できず、理由は本規律の範囲（画面文言そのものを見ない）では
確かめられないまま残った。

### 開発者判断（2026-09-15）

**エラーの行は当面、適合の比較の対象から外す**（バナーの行・最下行
（ファンクションキー表示行）と同じ扱いにする）。理由: 件数差の原因が
上記の診断・マニュアル確認を尽くしても特定できず、これ以上は禁止事項7
（画面本文を見ない）の制約下では追えないため。既知の差として本ノートに
記録し、自作ROMは引き続きマニュアルの書き方どおりにエラーメッセージを
出す実装のままとする。この判断はP1〜P5の測り直し（`l4-c2b`、下記事前登録
参照）には影響しない。

測り直しの事前登録: [l4-c2b-print-conformance-rerun-preregistration](l4-c2b-print-conformance-rerun-preregistration.md)。
