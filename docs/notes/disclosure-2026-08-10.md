...
（2026-08-10、これらのiologを全て含む）を指している。

**public になった正確な時刻は特定できる。** `.git/logs/refs/remotes/origin/main`
（push の記録）を `623e1cf` の包含関係で走査すると、初めてこれを含んだ push は
**2026-08-07 14:24:50（`788c7136`）**である。同日 11:44:13 の初回 push には
含まれていない。つまり
**2026-08-07 14:24:50 から本記録作成（2026-08-10）まで、約3日間、
未加工のデータポート値列が public に push された状態だった。**

（推定ではなく reflog の実測であることを明記しておく。再現手順は
`git merge-base --is-ancestor 623e1cf <各pushのnew値>` を古い順に試し、
最初に真を返すエントリのタイムスタンプを採る。）
