...
- `ext1-rom-bank-preregistration.md` — ext1 — 拡張ROMバンクの読み分け・欠落・割り込み安全性・中継費用 — 事前登録（改訂1・追補1）。配置案C選択を受け、自作試験ROM（識別パターン入りN88_0〜3.ROM＋中継ルーチン）だけでQ1〜Q4を確かめる。8腕×2走、陽性/陰性対照・故障注入・G1〜G6・関門P
- `ext1-rom-bank-results.md` — ext1 結果。8腕×2走すべて決定論的に一致。読み分け・復帰・無関係ビット保持・欠落時0xFF・中継の参考コスト(clock差分2)は成立、割り込みハンドラを窓の中に置く禁止設計は200往復中76で完全停止(broken)して禁止設計の実演も成立。設計結論5点を記録
...
- `disclosure-2026-09-19.md` — 開示記録 2026-09-19（l4-s1h測定担当が`q88measure`を`--out`無しで直接実行し画面本文が作業端末へ表示。m7hkの4件目。発生源（`write_report()`の標準出力経路）を塞ぎ自己検査を追加）
