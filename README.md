...
M6（L3 サービスルーチン）はいったん完了した（2026-09-14）。M7（L4 BASIC 互換処理系）は
着手済み（2026-09-15）で、ディスク無しの直接モード（打鍵エコー・`PRINT`）が公式と一致する
ところまで進んだ後、**ゴール A（代表的な BASIC プログラムが動く）の達成判定を得た
（2026-09-16）**。自分で書いた代表プログラム8本（九九・素数・配列のバブルソート・
フィボナッチ・階乗・文字列処理・`GOSUB`サブルーチン・`INPUT`を使う計算、いずれもテキスト
画面のみ）を、公式ROMで走らせた画面を期待値として実装より先に固定し、自作ROMで8本すべて
一致した。判定の範囲と限界は下表参照。L1 IPL と L2 フォントも完了しており、どちらも
**公式 ROM が無くても検証できる**（`tools/verify_l1.sh` / `tools/verify_l2.sh`）。
...
| M6 | L3 サービスルーチン | いったん完了（2026-09-14）。自作サブROM（`DISK.ROM`）を公式mainと組ませ、測定した約20の需要入口で、mainが受け取るデータ列と画面が公式と一致（適合テスト `tools/conform_l3.sh`。期待値は件数と SHA-256 のみ）。例外と再開条件は [docs/PLAN.md](docs/PLAN.md) 8節 |
| M7 | L4 BASIC 互換処理系 | 着手（2026-09-15）。自作アセンブラで組んだ自作 main ROM が、起動→自作バナー→`Ok`、直接モードの打鍵エコー（`tools/conform_l4.sh`、公式11腕・自作11腕とも conform）、`PRINT`（全16腕 print_matches）まで公式と一致。自作ROM側の照合は公式ROM無しでも回る。段階4（浮動小数点）は書式・丸め・定数読み取りを測定で確定し（仕様書 `docs/spec/l4-basic.md` 第3.7版）、単精度を実装（適合テスト `l4-c3` の単精度群17腕が conform）。倍精度は算術・型変換・定数読み取りまで実装し、出力書式は実装中。定数の読み取りは測定で1つに決まらず、実測に最も合う規則を推定として採用している（単精度 REP01 は57件中55件、倍精度 DREP10A は数値の出た全腕を再現）。段階5（プログラムモード）は行の入力・保存・`LIST`・`NEW`・`RUN`と流れの制御を実装した（仕様書 `docs/spec/l4-program.md` 第7版）。ゴールAの達成判定に使う代表プログラム群は、自分で書いた典型プログラム集（`tests/programs/`、テキスト画面のみ、8本）に決め（開発者判断、2026-09-15）、**その8本すべてが公式ROMの画面と一致し、ゴールAの達成判定を得た（適合テスト`l4-c5`、`tools/conform_l4.sh`のPROGRAM場面8/8 conform、2026-09-16）。** 打鍵エコー11腕・直接モード`PRINT`16腕・浮動小数点25腕も引き続き全conform。判定はテキスト画面の一致に限り、グラフィック・音・ディスク入出力・`RND`・`PRINT USING`・割り切れない数値関数の値は対象外で、実行の速さも比較対象に含めない。定数の読み取りは実測に最も合う規則を推定として採用している（単精度`REP01`は57件中55件、倍精度`DREP10A`）。詳細は [docs/PLAN.md](docs/PLAN.md) 8節 |
| M8 | 適合性テストスイート公開 | 適合テスト `tools/conform_l3.sh` が約20場面と適合条件1〜5を判定する形になった（2026-09-14 時点）。公式ROM保有者向けの手順書は未整備 |
| M9 | ゴール B（番地互換） | 未着手 |

### できないこと（正直に）

ゴールAは「代表的なテキスト画面のBASICプログラムが動く」の範囲であって、
N88-BASICの全機能ではない。2026-09-16時点で以下は未対応・未検証のまま：

- グラフィック・音・ディスク入出力（ディスク系はM6の需要入口ベースで別途進行中、M7側の自作mainからはまだ繋いでいない）
- `RND`、`PRINT USING`、`SQR`・`SIN`・`COS`・`EXP`・`LOG`などの大きい数値関数
- `WIDTH 40`の画面（`WIDTH 80,25`は測定・実装済み）
- 実行の速さ（自作ROMは公式ROMより遅い場合があり、比較対象にしていない）
- 浮動小数点の定数の読み取り（FIN）は測定で一意の規則に決まらず、実測に最も合う
  規則を推定として採用している（単精度`REP01`は57件中55件、倍精度`DREP10A`は
  数値の出た全腕を再現。詳細は`docs/spec/l4-basic.md`）

各仕様書の「未確定」節（`docs/spec/l4-basic.md`第10節・`docs/spec/l4-program.md`
第8節）に、測定できていない項目の一覧がある。
...
```
python3 src/build_main_rom.py <出力先ディレクトリ>   # N88.ROM/DISK.ROM/FONT.ROMを組み立てる
