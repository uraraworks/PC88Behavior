...
| M6 | L3 サービスルーチン | 進行中。サブ ROM（`DISK.ROM`）の仕様第4版、自作サブ ROM が自己検証を通過。二層検証（自己検証 `tools/verify_l3.sh` / 適合テスト `tools/conform_l3.sh`、期待値は件数と SHA-256 のみ）を追加。適合テスト本体が公式環境で合格（2026-08-11、適合条件1のみ） |
