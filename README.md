...
| M6 | L3 サービスルーチン | 進行中。サブ ROM（`DISK.ROM`）の仕様第12版、自作サブ ROM が自己検証を通過。二層検証（自己検証 `tools/verify_l3.sh` / 適合テスト `tools/conform_l3.sh`、期待値は件数と SHA-256 のみ）を追加。適合テスト本体が公式環境で合格（2026-08-11、適合条件1のみ）。混成ROMモード（公式main一式＋自作サブROM）で diskA 起動シーケンスを通す作業が進行中。main 側の構造的一致プレフィックスは 0 件→180 件まで前進、次の課題はラウンド境界を無視した受信バイトの取り違え（詳細は [docs/notes/m6k-mixed-divergence.md](docs/notes/m6k-mixed-divergence.md)） |
