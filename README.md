...
| M7 | L4 BASIC 互換処理系 | 着手（2026-09-15）。自作アセンブラで組んだ自作 main ROM が、起動→自作バナー→`Ok`、直接モードの打鍵エコー（`tools/conform_l4.sh`、公式11腕・自作11腕とも conform）、`PRINT`（全16腕 print_matches）まで公式と一致。自作ROM側の照合は公式ROM無しでも回る。次はプログラムモードと数値部（浮動小数点）。詳細は [docs/PLAN.md](docs/PLAN.md) 8節 |
