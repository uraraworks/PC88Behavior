...
def emit_font_sample(a, base=TEXT_BASE, cols=TEXT_COLS, stride=TEXT_STRIDE):
    ...
    アトリビュートバイト（1行あたり ATTR_BYTES バイト、文字域の直後）を
    書く直前に明示的にAを0クリアする処理は無い。xor_a()（A=0）は全体の
    先頭で1回だけ呼び、その後は各行「アトリビュート40バイトを書く→文字
    80バイトを書く」をそのままなぞるだけなので、実際に0になるのは1行目の
    アトリビュートだけである。2行目以降のアトリビュート域には、直前の行の
    最後の文字コードがAに残ったままの値がそのまま書かれる（RAM初期値に
    依存はしないが、行ごとの明示的な0クリアでもない）。この事実は
    vram_dump_selftest.sh / mem_write_log_selftest.sh を実測して初めて
    分かった（コミット 04201be）。
    """
