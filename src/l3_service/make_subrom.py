...
この1バイトの値がmain側の分岐に影響することが分かった。

そこで第9版（値はFDCから得る）と第20版（この場でFDCを新たに叩かない）
の両方を満たす構造にした: `LAST_FDC_RESULT`というRAM上の1バイトに、
`FDC_IN`が結果フェーズで実際に`$FB`から読み取るたびの値を保持して
おき（起動時init:MAIN_LOOP、保持:FDC_IN、下のFDC_IN実装参照）、
1バイト応答が必要になったときはFDCへ新規アクセスせずこの保持値を
そのまま送る。既存ログ（`measurements/m6c-sub-d0-boot.iolog.txt.gz`
のsub節、`OUT $FD`直前）で、直近のFDCポートアクセスは
起動時FDC初期化バッチ内の`SENSE INTERRUPT STATUS`結果2バイト目
（`IN $FB`）であり、それ以降`OUT $FD`まで追加のFDCアクセスは
無いことを確認した——「直近にFDC_INが読んだ値を持ち越す」構造は
この実走と矛盾しない。

**`LAST_FDC_RESULT`は「正しい値の推測」ではない。** 実際にFDCから
得た値をそのまま保持しているだけであり、6節14項が禁じる「でっちあげ」
には当たらない。ただし「どの局面でも直近のFDC_IN結果を汎用的に
持ち越してよいか」という一般則自体は、d0-bootの1ケースから
一般化した選択であり、他の局面まで測定の裏は取れていない
（3節に明記）。この1バイトの正しさ（適合条件1のハッシュ一致）は
引き続き適合条件の対象外。次にこの先へ進むには、この応答バイトの
中身を別の手段（伏せ字を緩めない範囲での追加測定）で確定させる
必要がある（3節に追記）。
...
# ---- 第21版で追加。FDC_INが実際にμPD765/8272から読み取った最新の
#      結果バイトを保持する。SEND_DISPATCH_IDLE/IDLE_SEND_BRANCHが
#      1バイト応答を送る際、ここに保持された値をそのまま送る（下の
#      該当ラベルのdocstring参照）。起動直後（一度もFDC_INが結果
#      フェーズを読んでいない状態）は0x00で初期化する——これは値の
#      推測ではなく「まだ実測値が無い」ことを表す暫定初期値。 ----
LAST_FDC_RESULT = 0x4307  # 1バイト: FDC_INが最後に読んだ$FBの値
...
def build_subrom(break_response=False, break_dispatch_return=False,
    ...
    # 第21版で追加: 実際に読み取れた結果バイトをLAST_FDC_RESULTへ
    # 保持する。中断（_fdc_in_abort/_fdc_in_aborted）経由でここに
    # 来ていない＝実際に$FBから読めた値のみを保持し、中断時は
    # 直前の保持値をそのまま残す（何も読めていないので上書きしない）。
    a.ld_mem_a(LAST_FDC_RESULT)
    a.ret()
    ...
    if break_dispatch_return:
        ...
        # 第21版で下のSEND_DISPATCH_IDLEと揃えて再訂正: この場面では
        # FDCを叩かず、LAST_FDC_RESULT（過去にFDC_INが実際に読み取った
        # 結果バイト）をそのまま送る（この分岐は--break-dispatch-return
        # の回帰専用でありディスパッチ復帰の有無だけを検出対象にして
        # いるため、送る値自体はどちらでもよいが本番構造と揃える）。
        a.ld_a_mem(LAST_FDC_RESULT)
        a.call("SEND_BYTE")
        a.jr("IDLE_DISPATCH")
    ...
    a.ld_mem_a(RUN_LEN)
    # 第21版で追加: LAST_FDC_RESULTの起動時初期値。まだ一度もFDC_INが
    # 結果を読んでいない段階の暫定値であり、値の推測ではない
    # （下のLAST_FDC_RESULT定義・SEND_DISPATCH_IDLEのdocstring参照）。
    a.ld_mem_a(LAST_FDC_RESULT)
    ...
    # 第20版で訂正: 応答フェーズ外（8バイトヘッダがまだ揃っていない
    # 段階、たとえば仕様書1.18節のラウンド#0のような短い要求形式）で
    # 1バイト応答を求められた場合、第9版はFDC_SENSE_DRIVE_STATUSを
    # 実発行してST3を送る構造にしていた。しかし混成ROM実走診断
    # （22:16版、分岐点103）で、**公式subはこの場面でFDCを一切叩かず、
    # その場で1バイト送るだけ**であることが構造として確定した
    # （基準側: IN $FE×2の直後にOUT $FDのみ。$FA/$FBへのアクセスは
    # 無い）。第9版の「FDCへ問い合わせる」構造自体がここでは誤りだった。
    #
    # 第21版で再訂正: 第20版はFDCを叩かない構造には合わせたが、
    # 送る値を無意味な0x00固定に戻したところ、実走診断（main側の
    # 構造一致件数）が第9版より後退した（258→102件、m6t参照）。
    # 「構造さえ合わせれば値は何でもよい」という想定はここでは
    # 成り立たなかった。
    #
    # そこで両者を両立させる: FDCへ**この場で新たに**問い合わせる
    # ことはしない（第20版で確定した構造を守る）が、送る値は
    # LAST_FDC_RESULT——過去にFDC_INが実際に$FBから読み取った
    # 結果バイトをRAMに保持したもの（上のLAST_FDC_RESULT定義参照）
    # ——を使う。これは「でっちあげた値」ではなく実際にFDCから得た
    # 値であり（6節14項）、かつこの場でFDCへアクセスしない
    # （分岐点103の構造と矛盾しない）。
    #
    # 既存ログ（measurements/m6c-sub-d0-boot.iolog.txt.gz、sub節、
    # seq7243のOUT $FD直前）で確認できる構造: 直近のFDC結果フェーズ
    # アクセスはseq7215のIN $FB（起動時FDC初期化バッチの、直前の
    # RECALIBRATEに続くSENSE INTERRUPT STATUS結果2バイト目）であり、
    # それ以降OUT $FDまでFDCポートへの追加アクセスは無い。
    # 「直近にFDC_INが読んだ値をそのまま持ち越す」構造はこの実走と
    # 矛盾しない。
    #
    # ただし「直近のFDC_IN結果を汎用的に保持する」という設計自体は、
    # このd0-bootの1ケースだけから一般化した選択であり、他の局面
    # （どのコマンドの何バイト目が直近かはRECV_DISPATCHの経路ごとに
    # 異なりうる）まで測定で裏を取れてはいない。6節14項が許すのは
    # 「実際にFDCから得た値を返す」ことであり、「どの値を持ち越すか」
    # の一般則までは未確定（仕様書3節に明記）。
    a.ld_a_mem(LAST_FDC_RESULT)
    a.call("SEND_BYTE")
    a.jp("IDLE_DISPATCH")   # 遠方分岐のためJP
