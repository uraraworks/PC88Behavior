...
# 内のツールでpython3だけで完結する（外部依存を増やさない）。ただし本
# ファイル自身の方針（冒頭docstring）は「外部依存ゼロで、第三者が
# `python3 make_subrom.py <出力先>` だけで同じROMを再生成できる」ことなので、
# tools/asm/ を伴わずにこのファイル単体をコピーして実行してもROM生成その
# ものは動かなければならない。見つかれば使い、無ければ --emit-asm 未使用時
# は無害なフォールバックで動作し、使用時だけ分かりやすいエラーで終了する。
...
try:
    ...
    def render_asm(asm_obj, header_comment=""):
        raise RuntimeError(
            "--emit-asm には tools/asm/asm_emit.py が必要です"
...
def build_subrom(break_write_ack=False,
                  ...
                  inject_m6ie_single_read=False,
                  inject_m6ie_nops_only=False,
    ...
    #
    # 第188版・m7kr: 読めない対象に対する再試行（仕様書1.58節）。
    # 第196版・m7le/m7lf で採用した。m7ks でこの実装は適合を壊したが、原因は再試行ではなく
    # 生成器の窓の関門の穴と余白9バイトだった（m7kw・m7ky・m7lb で確定）。関門を直し（m7lc）、
    # 余白を作った（m7ld）あとで、m7kr の合格条件7項目をそのまま使って再挑戦し、すべて真だった。
    # 1.58節は「読めない媒体」で1つの送信単位の内側に
    # `SEEK → SENSE INTERRUPT STATUS → SENSE DRIVE STATUS → READ DATA ×9`
    # が現れ、「読める媒体」ではREAD DATAが1件であることを記述している
    # （骨格）。ここではその2点に**同時に**合う最小の形として、
    # 「READ DATAを発行し、失敗したら（1回ごとにFDC_BEGINでやり直して）
    # 再試行し、最大9件で打ち切る。成功したらそこで止める」を実装する。
    # SEEK・SENSE INTERRUPT STATUS・SENSE DRIVE STATUSは呼び出し元
    # （このルーチンの外）で単位あたり1回のまま——ここで繰り返すのは
    # READ DATAだけである。
    #
    # **「9」は規則が書けたわけではない。** 1.58節が「9が依存しない
    # もの」として列挙した10軸（トラックあたりセクタ数・シリンダ数・
    # ヘッド数・セクタ番号の起点・失敗の原因・要求の入口・実行量・
    # FDCの結果ステータス・sub側ポーリング回数・打鍵時刻。同節の表）の
    # どれを振っても9は動かなかった、というところまでしか分かっていない。
    # 「9はこう決まる」という規則そのものは未確定であり、この9は
    # **観測された構造に形だけ合わせた値**である。
    #
    # **「失敗したら再試行する」という因果も測っていない。** 1.58節が
    # 測ったのは「9件の結果はすべて同じ」（m7ki）と「9件は同じ対象への
    # 要求」（m7jk）までで、公式が実際に1件ずつ結果を見て次を出すか
    # どうかは確認していない。ここでの「再試行」は、同じ対象へ9件が
    # 並ぶという観測から**推測して**当てはめた形である。
    #
    # 規則が測定から書けたとき、または1.58節の「9が依存しないもの」に
    # 9を動かす軸が見つかったときは、この実装とコメントを更新する
    # （更新の条件はdocs/notes/m7kr-implement-retry-decision.md）。
    # それまでこの実装は「観測に合わせただけ」であり、仕様として
    # 確定していない。
    a.ld_c(9)                           # 最大9回（未確定。上記コメント参照）
    a.label("_read_retry")
    ...
    # 再試行カウンタ(C)も同じ理由でこのループをまたいで保持される。
    ...
    # IN $F8（TC）の三つ組みを発行する。
    #
    # 第188版・m7kr: 成功／失敗の判定はFDC_ABORTを流用する。m7cmの通り
    # （下のSEND_DISPATCH_ONE_BYTEのコメント参照）、データフェーズ256件を
    # 読み切れずに結果フェーズへ直行した場合にFDC_BEGINがクリアした
    ...
    elif inject_m6ie_single_read:
        a.in_port(P_PIO_C)
        for _ in range(4):
            a.nop()
    elif inject_m6ie_nops_only:
        for _ in range(6):
            a.nop()
    a.jp("IDLE_DISPATCH")
...
def build(break_write_ack=False,
    ...
    for _attempt in range(MAX_ALIGN_PADDING_ATTEMPTS + 1):
        a = build_subrom(break_write_ack=break_write_ack,
                          ...
                          inject_m6ie_single_read=inject_m6ie_single_read,
                          inject_m6ie_nops_only=inject_m6ie_nops_only,
...
def main():
    ...
    ap.add_argument("--inject-m6ie-single-read", action="store_true",
                    help="m6i-e E2: ラウンド#0応答直後へIN $FEとNOP 4個を挿入する")
    ap.add_argument("--inject-m6ie-nops-only", action="store_true",
                    help="m6i-e E3: ラウンド#0応答直後へNOP 6個を挿入する")
    ...
    if sum((args.inject_m6ib_b1, args.inject_m6ib_b2, args.inject_m6ib_b3,
            args.inject_m6ib_b4, args.inject_m6ib_b5,
            args.inject_m6ie_single_read, args.inject_m6ie_nops_only)) > 1:
    ...
    if args.emit_asm is not None and not _ASM_EMIT_AVAILABLE:
        print("エラー: --emit-asm には tools/asm/asm_emit.py が必要です"
