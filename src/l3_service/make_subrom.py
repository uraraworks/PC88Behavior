...
中身は伏せ字済みで読めない）。ここが規律上の難所: 6節14項の
趣旨（値を推測で埋めない）を保ちながら構造だけを合わせるには、
値に意味を持たせない`0x00`を暫定的に置く以外に取りうる手が無い。
**`0x00`は「正しい値の推測」ではなく「値を持たせないための選択」**
であることを明記する——0x00がST3の妥当なパターンだからでも、
ディスク無し状態を意味させたいからでもない。この1バイトの正しさは
引き続き適合条件の対象外（6節14項の原則を維持）。次にこの先へ
進むには、この応答バイトの中身を別の手段（伏せ字を緩めない範囲での
追加測定）で確定させる必要がある（3節に追記）。
...
def build_subrom(break_response=False, break_dispatch_return=False,
    ...
    if break_dispatch_return:
        ...
        # 第20版で下のSEND_DISPATCH_IDLEと揃えて訂正: この場面ではFDCを
        # 叩かず、意味を持たせない暫定値0x00をそのまま送る（この分岐は
        # --break-dispatch-return の回帰専用でありディスパッチ復帰の
        # 有無だけを検出対象にしているため、送る値自体はどちらでもよいが
        # 本番構造と揃える）。
        a.ld_a(0x00)   # 暫定値。SEND_DISPATCH_IDLEと同じ理由（値は未確定）
        a.call("SEND_BYTE")
        a.jr("IDLE_DISPATCH")
    ...
    #
    # 構造は直すが、送るべき値は依然として未確定（1.18節・3節）。
    # 6節14項の趣旨（値を推測で埋めない）は変わらないので、意味を
    # 持たせないための暫定値として0x00を置く。**0x00が正しい/尤もらしい
    # 値だからではない**——むしろ意味を持たせると推測になるため、
    # 最も無色な値を選んだだけである。この1バイトの正しさ（適合条件1の
    # ハッシュ一致）はこの変更の目標に含まれない。
    a.ld_a(0x00)   # 暫定値。意味を持たせないための選択（値は未確定、仕様書3節参照）
    a.call("SEND_BYTE")
    a.jp("IDLE_DISPATCH")   # 遠方分岐のためJP
