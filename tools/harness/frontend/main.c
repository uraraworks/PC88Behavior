 ...
 * 英字は小文字の RETROK コード（SHIFT なし）として送る。
 * 【訂正 2026-09-24】以前ここに「大文字として入る」と書いていたが誤りだった。
 * m6f-b（docs/notes/m6f-b-directory-entry-fields-results.md §5）で、この経路で
 * 打った英字は ROM へ**小文字の符号**として届くことを確かめた（打鍵した
 * OPEN "QZ7A" のファイル名がディスク上で小文字だった一方、CHR$ で大文字の
