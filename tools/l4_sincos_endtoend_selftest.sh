#!/usr/bin/env bash
# tools/l4_sincos_endtoend_selftest.sh — SIN/COS/TAN(第4.16a節)のBASIC
# 呼び出し経路end-to-end検査(陰性対照つき)。**公式ROMは要らない**
# (自作ROMだけで完結する)。tools/l4_sqr_endtoend_selftest.shと同じ設計
# (docs/notes/l4-c8-transcendental-conformance-scene-results.mdで見つかった
# EXT_BANK_CALLのBC/DE退避不具合の再発防止と同じ検査方法)を踏襲する。
#
# 背景: tools/l4_sincos_bank_conform.py(バンクルーチン単体のバイト照合)
# だけでは、`interp.asm FTNF_DO_SIN/COS/TAN`→`EXT_BANK_CALL`
# (src/ext_bank/relay.asm)→バンク0`EXT_BANK0_SIN/COS/TAN_ENTRY`
# (src/ext_bank/bank0.asm)という実際の呼び出し経路が正しく往復する
# ことまでは確認できない(SQRがまさにこれでハングを見落とした前例、
# l4_sqr_endtoend_selftest.sh冒頭コメント参照)。TANはSIN・COSの結果を
# 使って自分の中でさらにEXT_BANK0_SIN_ENTRY相当の処理を呼ぶため
# (SC_TAN_IMPL、bank0.asm)、単発のSIN/COSより窓の状態遷移が複雑で、
# この検査で確かめる価値が高い。
#
# 検査:
#   1. 通常ビルドで`print sin(1)\nprint cos(1)\nprint tan(1)\n`を打ち、
#      期待どおり出力セルが変化すること(cell_count>0、ハングしていれば0)。
...
#      実際に壊れること(検出力の確認。この故障注入自体はSQRの検査で
#      既に確認済みの仕組みだが、SIN/COS/TAN経路でも同じ検出力がある
#      ことを独立に確かめる)。
#
...
#
# 使い方: tools/l4_sincos_endtoend_selftest.sh
...
# l4_sqr_endtoend_selftest.shと同じ走行フレーム構成(dump=824,run=1024,
# before=690)。打鍵文字列は3行(SIN/COS/TANそれぞれ1回ずつ)。
TYPED='print sin(1)\nprint cos(1)\nprint tan(1)\n'
BEFORE=690
DUMP=824
RUN=1200   # 3関数分(TANは内部でSIN+COS相当を2回計算する)、SQRのRUN=1024より余裕を見る
...
# l4_sqr_endtoend_selftest.shと同じ状態機械
...
# -----------------------------------------------------------------------
say "1. 通常ビルドで print sin(1)/cos(1)/tan(1) が正しく完了すること"
...
if [ "$FAILED" -eq 0 ]; then
  echo
  echo "l4_sincos_endtoend_selftest: OK"
else
  echo
  echo "l4_sincos_endtoend_selftest: NG" >&2
fi
exit "$FAILED"
