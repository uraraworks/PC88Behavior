...
def _sector_payload(c: int, h: int, r: int, fat_value: int, filler: int) -> bytes:
    value = fat_value if (c, h, r) in ALLOCATION_TABLE_COORDS else filler
    return bytes([value]) * SECTOR_SIZE


def build_track(c: int, h: int, fat_value: int, filler: int) -> bytes:
    ...
    for r in range(SECTOR_BASE, SECTOR_BASE + SECTORS_PER_TRACK):
        ...
        body += hdr
        body += _sector_payload(c, h, r, fat_value, filler)
    return bytes(body)


def build_blank_disk(fat_value: int, filler: int) -> bytes:
    """事前登録 第2節の規則だけで公開D88像を作る。"""
    ...
    for c in range(CYLINDERS):
        for h in range(HEADS):
            trk = build_track(c, h, fat_value, filler)
            phys = c * 2 + h
...
def main() -> int:
    ...
    data = build_blank_disk(args.fat_value, args.filler)
