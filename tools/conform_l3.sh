...
cat <<'EOF'
  ...
  適合条件1・2・4・5は、このスクリプトの各判定と対照検査により判定する。
  2026-08-23の公式環境実走では4条件とも合格した（条件2はこの実走が初判定）。

  未判定で残るのは条件3だけである。自作サブROMは割り込みを使わず、受理点が
  0件のため、条件3の判定対象が存在しない。空集合を合格とは扱わず、判定不能と
  明示する。判定器の陽性・陰性対照と、同条件の公式subでの判定は上記のとおり。
  根拠: docs/notes/m7cb-official-conformance-after-compression.md
EOF
