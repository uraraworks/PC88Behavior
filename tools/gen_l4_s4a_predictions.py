#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gen_l4_s4a_predictions.py — l4_mbf_oracle.py を使って l4-s4a の32腕の
出力を事前登録する docs/notes/l4-s4a-gwbasic-predictions.tsv を生成する。

腕の打鍵文字列は、l4-s4a 事前登録担当が定義した32腕表と同一である
(id・typed列)。ここではその定義をそのまま埋め込み、予測列
(kind・predicted)だけを tools/l4_mbf_oracle.py で計算する。

使い方:
    python3 tools/gen_l4_s4a_predictions.py > docs/notes/l4-s4a-gwbasic-predictions.tsv
"""
...
from tools.l4_mbf_oracle import predict  # noqa: E402
...
def main() -> int:
    ...
    oracle_path = os.path.join(here, "l4_mbf_oracle.py")
    digest = hashlib.sha256(open(oracle_path, "rb").read()).hexdigest()

    print(f"# 生成元: tools/l4_mbf_oracle.py sha256={digest}")
    print("# 生成コマンド: python3 tools/gen_l4_s4a_predictions.py")
    print("id\ttyped\tkind\tpredicted")
    for arm_id, _group, typed in ARMS:
        body = typed
        if body.lower().startswith("print "):
            body = body[len("print "):]
        kind, pred = predict(body)
        print(f"{arm_id}\t{typed}\t{kind}\t\"{pred}\"")
    return 0
